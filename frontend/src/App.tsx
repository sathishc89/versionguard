...
type Dialog = 'create' | 'upload' | 'history' | 'warning' | 'delete' | 'success' | null;
...
export default function App({ config, onSignOut }: { config: RuntimeConfig; onSignOut?: () => void }) {
  ...
  const openDelete = (document: DocumentRecord) => { setDeleteDocument(document); setDialog('delete'); };
  ...
  const removeDocument = async () => {
    ...
    setBusy(true);
    try { await api(`/documents/${deleteDocument.documentId}`, { method: 'DELETE' }); setDialog(null); setDeleteDocument(null); setToast('Workspace deleted'); await refresh(); }
    catch (caught) { setToast((caught as Error).message); }
  ...
  return <main className="app-shell">
    ...
    {dialog === 'delete' && <Dialog title="Delete workspace" onClose={closeDialog} warning><div className="warning-box"><span className="warning-symbol">!</span><p>This permanently deletes <strong>{deleteDocument?.name}</strong>, all of its versions, and the stored files.</p></div><div className="dialog-actions"><button className="ghost-button" onClick={closeDialog}>Cancel</button><button className="danger-button" onClick={() => void removeDocument()} disabled={busy}>{busy ? 'Deleting...' : 'Delete workspace'}</button></div></Dialog>}
